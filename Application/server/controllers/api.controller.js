...
const sendRequestsController = {
  ...
  hideSend: async (req, res) => {
    try {
      ...
      let bufferPair = await helperFunctions.fetchAndConvert(
        coverImageSource,
        secretImageSource
      );
...
const helperFunctions = {
  ...
  fetchAndConvert: async (coverImageUrl, secretImageUrl) => {
    let pngBuffers = {};
    try {
      const axiosResponseCover = await axios.get(coverImageUrl, {
        responseType: "arraybuffer",
      });
      const axiosResponseSecret = await axios.get(secretImageUrl, {
        responseType: "arraybuffer",
      });
      pngBuffers.coverBuffer = await sharp(axiosResponseCover.data)
        .toFormat("png")
        .toBuffer();
      pngBuffers.secretBuffer = await sharp(axiosResponseSecret.data)
        .toFormat("png")
        .toBuffer();
    } catch (error) {
  ...
  sendToFlask: async (data) => {
    try {
      ...
      form.append("coverImage", data.coverBuffer, {
        filename: "cover.png",
        contentType: "image/png",
      });
      form.append("secretImage", data.secretBuffer, {
        filename: "secret.png",
        contentType: "image/png",
      });
      const response = await axios.post(
        FLASK_SERVER_URL + "/test" /*'/create_stego_image'*/,
        form,
        {
          headers: {
            ...form.getHeaders(),
