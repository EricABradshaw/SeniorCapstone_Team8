...
from NSteGuz import preprocess_image, verbose, CustomModel
import os
import argparse
import time
import uuid
...
def output_png_old(outputDir, filename, image, isBatch=False):
    """
...
def create_stego_image(index=0, coverImage=None, secretImage=None):
    ...
    # Load the model
    CHECKPOINTS_PATH = inputModelPath    
    model = CustomModel()
    checkpoint = tf.train.Checkpoint(model=model)
    checkpoint.restore(tf.train.latest_checkpoint(CHECKPOINTS_PATH)).expect_partial()
